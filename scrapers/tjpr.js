...
// Portal: projudi.tjpr.jus.br (requer login). Via DataJud como fonte principal.
...
const DATAJUD_INDEX = 'api_publica_tjpr';
...
async function consultar(numero) {
  ...
  try {
    console.log(`[tjpr:datajud] Consultando ${numDigitos}`);
    ...
    if (!hits.length) {
      return { sucesso: false, erro: 'Processo não encontrado no TJPR (DataJud CNJ)', andamentos: [], tribunal: 'TJPR' };
    }
    const andamentos = parsearMovimentos(hits[0]._source?.movimentos || []);
    console.log(`[tjpr:datajud] ${andamentos.length} movimentos`);
    return { sucesso: true, andamentos, tribunal: 'TJPR', portal: 'datajud' };
  } catch (err) {
    console.error(`[tjpr:datajud] Erro:`, err.message);
    return { sucesso: false, erro: `Erro ao consultar TJPR: ${err.message}`, andamentos: [], tribunal: 'TJPR' };
  }
}
