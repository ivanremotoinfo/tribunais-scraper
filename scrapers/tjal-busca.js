// TJAL — eSAJ novo (UI azul/Softplan) — Busca por OAB
// Portal: https://www2.tjal.jus.br/cpopg/search.do
// OAB formato: {numero}AL — ex: "20000AL"
// Estrutura diferente do TJSP/TJCE: classes linkProcesso, classeProcesso, dataLocalDistribuicaoProcesso

const axios = require('axios');
const cheerio = require('cheerio');

const BASE    = 'https://www2.tjal.jus.br';
const UF      = 'AL';
const TIMEOUT = 25000;
...
// eSAJ redireciona para página de detalhe quando há exatamente 1 resultado
...
async function buscarPagina(oab, pagina) {
  ...
  console.log(`[tjal-busca] GET OAB=${oab} página=${pagina}`);
...
async function buscar({ oab }) {
  if (!oab) throw new Error('OAB é obrigatório para busca no TJAL');

  const oabNum = String(oab).replace(/\D/g, '');
  if (!oabNum) throw new Error('Número OAB inválido');

  const oabAL = `${oabNum}${UF}`;

  const html1 = await buscarPagina(oabAL, 1);

  if (semResultado(html1)) {
    return { sucesso: true, tribunal: 'TJAL', total: 0, processos: [] };
  }

  const unico = extrairProcessoUnico(html1);
  if (unico) {
    return { sucesso: true, tribunal: 'TJAL', total: 1, processos: unico };
  }
  ...
  console.log(`[tjal-busca] ${processos.length} processos na pág 1 de ${totalPags} (total: ${total})`);

  for (let p = 2; p <= Math.min(totalPags, 5); p++) {
    try {
      const htmlN = await buscarPagina(oabAL, p);
      const mais  = parsearPagina(htmlN);
      processos = processos.concat(mais);
      console.log(`[tjal-busca] pág ${p}: +${mais.length} → total ${processos.length}`);
    } catch (e) {
      console.warn(`[tjal-busca] pág ${p} falhou:`, e.message);
      break;
    }
  }

  return { sucesso: true, tribunal: 'TJAL', total: processos.length, processos };
}
