// TJMA — eSAJ novo (UI azul/Softplan) — Busca por OAB
// Portal: https://esaj.tjma.jus.br/cpopg/search.do
// OAB formato: {numero}MA — ex: "20000MA"

const axios = require('axios');
const cheerio = require('cheerio');

const BASE    = 'https://esaj.tjma.jus.br';
const UF      = 'MA';
const TIMEOUT = 25000;
...
async function buscarPagina(oab, pagina) {
  ...
  console.log('[tjma-busca] GET OAB=' + oab + ' página=' + pagina);
...
async function buscar({ oab }) {
  if (!oab) throw new Error('OAB é obrigatório para busca no TJMA');

  const oabNum = String(oab).replace(/\D/g, '');
  if (!oabNum) throw new Error('Número OAB inválido');

  const oabMA = `${oabNum}${UF}`;

  const html1 = await buscarPagina(oabMA, 1);

  if (semResultado(html1)) {
    return { sucesso: true, tribunal: 'TJMA', total: 0, processos: [] };
  }
  ...
  console.log('[tjma-busca] ' + processos.length + ' processos na pág 1 de ' + totalPags + ' (total: ' + total + ')');

  for (let p = 2; p <= Math.min(totalPags, 5); p++) {
    try {
      const htmlN = await buscarPagina(oabMA, p);
      const mais  = parsearPagina(htmlN);
      processos = processos.concat(mais);
    } catch (e) {
      console.warn('[tjma-busca] pág ' + p + ' falhou:', e.message);
      break;
    }
  }

  return { sucesso: true, tribunal: 'TJMA', total: processos.length, processos };
}
