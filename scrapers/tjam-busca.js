// TJAM — eSAJ novo (UI azul/Softplan) — Busca por OAB
// Portal: https://consultasaj.tjam.jus.br/cpopg/search.do
// OAB formato: {numero}AM — ex: "20000AM"
...
async function buscar({ oab }) {
  ...
  const oabAM = `${oabNum}${UF}`;

  const html1 = await buscarPagina(oabAM, 1);

  if (semResultado(html1)) {
    return { sucesso: true, tribunal: 'TJAM', total: 0, processos: [] };
  }

  const unico = extrairProcessoUnico(html1);
  if (unico) {
    return { sucesso: true, tribunal: 'TJAM', total: 1, processos: unico };
  }
  ...
  console.log(`[tjam-busca] ${processos.length} processos na pág 1 de ${totalPags} (total: ${total})`);
