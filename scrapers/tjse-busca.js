// TJSE — eSAJ novo (UI azul/Softplan) — Busca por OAB
// Portal: https://esaj.tjse.jus.br/cpopg/search.do
// OAB formato: {numero}SE — ex: "20000SE"

const axios = require('axios');
const cheerio = require('cheerio');

const BASE    = 'https://esaj.tjse.jus.br';
const UF      = 'SE';
const TIMEOUT = 25000;
...
async function buscarPagina(oab, pagina) {
  ...
  console.log('[tjse-busca] GET OAB=' + oab + ' página=' + pagina);
...
async function buscar({ oab }) {
  if (!oab) throw new Error('OAB é obrigatório para busca no TJSE');

  const oabNum = String(oab).replace(/\D/g, '');
  if (!oabNum) throw new Error('Número OAB inválido');

  const oabSE = `${oabNum}${UF}`;

  const html1 = await buscarPagina(oabSE, 1);

  if (semResultado(html1)) {
    return { sucesso: true, tribunal: 'TJSE', total: 0, processos: [] };
  }
  ...
  console.log('[tjse-busca] ' + processos.length + ' processos na pág 1 de ' + totalPags + ' (total: ' + total + ')');

  for (let p = 2; p <= Math.min(totalPags, 5); p++) {
    try {
      const htmlN = await buscarPagina(oabSE, p);
      const mais  = parsearPagina(htmlN);
      processos = processos.concat(mais);
    } catch (e) {
      console.warn('[tjse-busca] pág ' + p + ' falhou:', e.message);
      break;
    }
  }

  return { sucesso: true, tribunal: 'TJSE', total: processos.length, processos };
}
