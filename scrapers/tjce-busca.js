// TJCE — eSAJ — Busca por OAB (1º grau)
// Portal: https://esaj.tjce.jus.br/cpopg/search.do
// OAB formato: {numero}{UF} — ex: "50000CE"
// Estrutura idêntica ao TJSP eSAJ

const axios = require('axios');
const cheerio = require('cheerio');

const BASE    = 'https://esaj.tjce.jus.br';
const TIMEOUT = 25000;
...
async function buscarPagina(oab, pagina) {
  ...
  console.log(`[tjce-busca] GET OAB=${oab} página=${pagina}`);
...
async function buscar({ oab }) {
  if (!oab) throw new Error('OAB é obrigatório para busca no TJCE');

  const oabNum = String(oab).replace(/\D/g, '');
  if (!oabNum) throw new Error('Número OAB inválido');

  // Formato TJCE: número + UF, ex: "50000CE"
  const oabCE = `${oabNum}CE`;

  const html1 = await buscarPagina(oabCE, 1);
  let processos = parsearCards(html1);
  const totalPags = totalPaginas(html1);

  console.log(`[tjce-busca] ${processos.length} processos na pág 1 de ${totalPags}`);

  for (let p = 2; p <= Math.min(totalPags, 5); p++) {
    try {
      const htmlN = await buscarPagina(oabCE, p);
      const mais  = parsearCards(htmlN);
      processos = processos.concat(mais);
      console.log(`[tjce-busca] pág ${p}: +${mais.length} → total ${processos.length}`);
    } catch (e) {
      console.warn(`[tjce-busca] pág ${p} falhou:`, e.message);
      break;
    }
  }

  return { sucesso: true, tribunal: 'TJCE', total: processos.length, processos };
}
