// TJPB — eSAJ novo (UI azul/Softplan) — Busca por OAB
// Portal: https://esaj.tjpb.jus.br/cpopg/search.do
// OAB formato: {numero}PB — ex: "20000PB"

const axios = require('axios');
const cheerio = require('cheerio');

const BASE    = 'https://esaj.tjpb.jus.br';
const UF      = 'PB';
const TIMEOUT = 25000;
...
async function buscarPagina(oab, pagina) {
  ...
  console.log('[tjpb-busca] GET OAB=' + oab + ' página=' + pagina);
...
async function buscar({ oab }) {
  if (!oab) throw new Error('OAB é obrigatório para busca no TJPB');

  const oabNum = String(oab).replace(/\D/g, '');
  if (!oabNum) throw new Error('Número OAB inválido');

  const oabPB = `${oabNum}${UF}`;

  const html1 = await buscarPagina(oabPB, 1);

  if (semResultado(html1)) {
    return { sucesso: true, tribunal: 'TJPB', total: 0, processos: [] };
  }
  ...
  console.log('[tjpb-busca] ' + processos.length + ' processos na pág 1 de ' + totalPags + ' (total: ' + total + ')');

  for (let p = 2; p <= Math.min(totalPags, 5); p++) {
    try {
      const htmlN = await buscarPagina(oabPB, p);
      const mais  = parsearPagina(htmlN);
      processos = processos.concat(mais);
    } catch (e) {
      console.warn('[tjpb-busca] pág ' + p + ' falhou:', e.message);
      break;
    }
  }

  return { sucesso: true, tribunal: 'TJPB', total: processos.length, processos };
}
