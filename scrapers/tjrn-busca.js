// TJRN — eSAJ novo (UI azul/Softplan) — Busca por OAB
// Portal: https://esaj.tjrn.jus.br/cpopg/search.do
// OAB formato: {numero}RN — ex: "20000RN"

const axios = require('axios');
const cheerio = require('cheerio');

const BASE    = 'https://esaj.tjrn.jus.br';
const UF      = 'RN';
const TIMEOUT = 25000;
...
async function buscarPagina(oab, pagina) {
  ...
  console.log('[tjrn-busca] GET OAB=' + oab + ' página=' + pagina);
...
async function buscar({ oab }) {
  if (!oab) throw new Error('OAB é obrigatório para busca no TJRN');

  const oabNum = String(oab).replace(/\D/g, '');
  if (!oabNum) throw new Error('Número OAB inválido');

  const oabRN = `${oabNum}${UF}`;

  const html1 = await buscarPagina(oabRN, 1);

  if (semResultado(html1)) {
    return { sucesso: true, tribunal: 'TJRN', total: 0, processos: [] };
  }
  ...
  console.log('[tjrn-busca] ' + processos.length + ' processos na pág 1 de ' + totalPags + ' (total: ' + total + ')');

  for (let p = 2; p <= Math.min(totalPags, 5); p++) {
    try {
      const htmlN = await buscarPagina(oabRN, p);
      const mais  = parsearPagina(htmlN);
      processos = processos.concat(mais);
    } catch (e) {
      console.warn('[tjrn-busca] pág ' + p + ' falhou:', e.message);
      break;
    }
  }

  return { sucesso: true, tribunal: 'TJRN', total: processos.length, processos };
}
